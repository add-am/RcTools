...
By default, because this function is meant to directly follow the `value_to_score()` function, it will also detect if the supplied column name ends in "Score" and if so will replace it with "Grade". This just keeps the column names cleaner. Thus, the final output would look like this:
...
The `save_n3_table()` function is designed to format data frames into a specific style that is used in the Northern Three Report Cards. The function takes a data frame and formats it according to the specifications of the report card, including adding letter grades and adjusting the layout.
