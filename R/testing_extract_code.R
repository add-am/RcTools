...
Region <- sf::st_read("n3_region.gpkg") |> 
...
ereefs_extract <- function(Region, StartDate, EndDate, Variable, Downsample = 0){
  ...
  if (stringr::str_detect(Variable, "True Colour")) {
    ...
    nc_data <- purrr::map(colour_channels, \(x) {
      ...
      stars::read_ncdf(
        ...
        ncsub = cbind(
          ...
          count = c(num_of_rows, num_of_cols, 3)
        )
      )
    })
    ...
    nc_data <- purrr::map(nc_data, \(x) {
      ...
      x <- x * 15 #multiple base values by 10
      ...
      x[drop = TRUE] #drop the time values if there is only one time step
    })
    ...
    nc_data_1 <- merge(nc_data)

    #Issues with time values not being read properly after merge
    #solution so far is dont merge, and use rgb on prior data
    #this works but map looks washed out, possibly no need to use TCbright?
    #potential second solution is to be explicit about the bands i am targetting - try this first?

    library(tmap)

    #when acting on the merged product we just have to be more specific when the time dimension is present:
    tm_shape(nc_data_1) + tm_rgb(col = tm_vars("attributes", dimvalues = c("R_645", "R_555", "R_470"), multivariate = TRUE))

    #when acting on the do.call product the bands are kept as attributes and are thus not confused with the time dimension, can be less specific
    tm_shape(nc_data) + tm_rgb()

    #both seem to look the same, i.e. washed out. Consider the TcBright?




    stars::st_get_dimension_values(nc_data_1, "time")

nc_data_2 <- stars::st_set_dimensions(
  nc_data_1,
  "time",
  as.POSIXct(stars::st_get_dimension_values(nc_data_1, "time"), origin = "1970-01-01", tz = "UTC")
)
...
tm_shape(nc_data) + tm_rgb()
tm_shape(nc_data_1) + tm_rgb()
tm_shape(nc_data_1) + tm_raster()


stars::st_dimensions(nc_data)$time
stars::st_dimensions(nc_data_1)$time

stars::st_get_dimension_values(nc_data, "time")
stars::st_get_dimension_values(nc_data_1, "time")
