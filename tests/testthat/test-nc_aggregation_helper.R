test_that("Aggregation returns a smaller stars object (single attribute)", {
  ...
  #do the aggregations
  aggregated_data <- purrr::map(agg_options, \(x) nc_aggregation_helper(turb_reg, x, mean))
...
test_that("Aggregation returns a smaller stars object (multi-attribute)", {
  ...
  #do the aggregations
  aggregated_data <- purrr::map(agg_options, \(x) nc_aggregation_helper(tc_reg, x, mean))
